...
// independently.
template <typename IdxType>
COOMatrix CSRRowWisePickPadded(
  ...
  IdArray picked_col = IdArray::Empty({num_rows * num_picks}, idtype, ctx);
  IdArray picked_idx = IdArray::Empty({num_rows * num_picks}, idtype, ctx);
...
#pragma omp parallel num_threads(num_threads)
  {
    ...
    local_picked_col = IdArray::Empty({(start_i - end_i) * num_picks}, idtype, ctx);
    local_picked_idx = IdArray::Empty({(start_i - end_i) * num_picks}, idtype, ctx);    
    std::fill(local_picked_idx,local_picked_idx +(start_i - end_i) * num_picks,-1);
    ...
    for (int64_t i = start_i; i < end_i; ++i)
      {
        ...
        const IdxType local_i = start_i - i;
        const IdxType local_row_offset =  local_i * num_picks;
      
      
        if (len != 0)
          {
            const int64_t num_picks_actual = num_picks_fn(
                                                          rid, off,len, indices, data);
            pick_fn(
                    rid, off, len, num_picks_actual, indices, data, local_picked_idx + local_row_offset);
            for (int64_t j = 0; j < num_picks_actual; ++j) 
              {
                const IdxType picked = local_picked_idx[local_row_offset + j];
                local_picked_col[local_row_offset + j] = indices[picked];
              }
          }
      }
  }
